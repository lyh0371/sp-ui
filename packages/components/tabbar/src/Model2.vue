<template>
  <div :id="`sp-tabbar__${id}`" class="sp-tabbar2">
    <div class="tabbar-warp__indicator">
...
const clickHandle = (item: TtabList, index: Number, e: MouseEvent) => {
  ...
  if (targetDom.matches('.icon-warp')) {
    // 看看是不是目标元素
    x = targetDom.getBoundingClientRect().x
  } else if (targetDom.matches('.tabbar-warp__item')) {
    x = targetDom.querySelector('.icon-warp')!.getBoundingClientRect().x
  } else {
    // 点着儿子了
...
onMounted(() => {
  ...
  const tbx = doc!.querySelector('.tabbar-warp')?.getBoundingClientRect().x as number
  const firstItem = doc!.getElementsByClassName('tabbar-warp__item')[props.activeIndex]?.querySelector('.icon-warp')
  waveWidth.value = (doc!.querySelector('#wave')!.getBoundingClientRect().width - 46) as number
